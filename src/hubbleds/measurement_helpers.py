...
from hubbleds.utils import angular_size_for_velocity, distance_for_velocity, distance_from_angular_size, observed_wavelength_from_redshift, rest_wavelength, velocity_from_wavelengths, w2v


def measurements_needed(local_state: LocalState) -> int:
...
def fill_and_add_wavelengths(api: LocalAPI, local_state: Reactive[LocalState], global_state: Reactive[GlobalState]):
...
def fill_and_add_velocities(api: LocalAPI, local_state: Reactive[LocalState], global_state: Reactive[GlobalState]):
    measurements = [m for m in local_state.value.measurements]
    for measurement in measurements:
...
def fill_add_wave_vel_ang(api: LocalAPI, local_state: Reactive[LocalState], global_state: Reactive[GlobalState]):
    measurements = [m for m in local_state.value.measurements]
    for measurement in measurements:
        add_velocity(measurement)
        add_ang_size_value(measurement)
...
def fill_add_angular_size_and_distance(api: LocalAPI, local_state: Reactive[LocalState], global_state: Reactive[GlobalState]):
    measurements = [m for m in local_state.value.measurements]
    for measurement in measurements:
...
def fill_add_all_measurements(api: LocalAPI, local_state: Reactive[LocalState], global_state: Reactive[GlobalState]):
    measurements = [m for m in local_state.value.measurements]
    for measurement in measurements:
        add_velocity(measurement)
        add_distance(measurement)
